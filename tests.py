...
class TestBooksCollector:
    ...
    def test_add_new_book_add_new_book(self, collector, book_name):
        ...
        assert book_name in collector.get_books_genre(), (f'Книга "{book_name}" не добавлена в список! Длина названия'
                                                          f' - {len(book_name)}.')
    ...
    def test_add_new_book_add_book_with_incorrect_name(self, collector, book_name):
        collector.add_new_book(book_name)
        assert book_name not in collector.get_books_genre(), (f'Книга "{book_name}" не добавлена в список! Длина '
                                                              f'названия - {len(book_name)}.')
    ...
    def test_set_book_genre_incorrect_name_book(self, collector):
        ...
        assert 'Репка' not in collector.get_books_genre()
    ...
    def test_get_books_with_specific_genre_return_books_with_specific_genre(self, collector):
        ...
        assert ('Чук и Гек', 'Сказки' in collector.get_books_with_specific_genre('Ужасы')
                             and len(collector.get_books_with_specific_genre('Ужасы')) == 2)
    ...
    def test_get_books_genre_return_books_genre(self, collector):
        ...
        assert 'Солярис' in collector.get_books_genre() and len(collector.get_books_genre()) == 1
    ...
    def test_get_books_for_children_return_books_for_children(self, collector):
        ...
        assert ('Черная комната' not in collector.get_books_for_children()
                and 'Красная шапочка' in collector.get_books_for_children()
    ...
    def test_get_list_of_favorites_books_return_list_of_favorites_books(self, collector):
        ...
        assert ('Дэнс дэнс дэнс' in collector.get_list_of_favorites_books()
                and len(collector.get_list_of_favorites_books()) == 1)

    # test_016 - позитивный - у вновь добавленной книги не установлен жанр
    def test_add_new_book_no_genre(self, collector):
        collector.add_new_book('Проверка микрофона')
        assert collector.get_book_genre('Проверка микрофона') == ''
